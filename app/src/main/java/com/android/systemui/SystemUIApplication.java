...
public class SystemUIApplication extends Application implements SysUiServiceProvider {
    private static SystemUIApplication mApplication;
    ...
    public void onCreate() {
        ...
        if(SystemProperties.get("ro.product.platform").equals("homlet")){
            Log.d(TAG,"is homlet");
            mBaseStartServices.add("com.android.systemui.Dependency$DependencyCreator");
        ...
        bootToostFilter.addAction(Intent.ACTION_SCREEN_OFF);
        bootToostFilter.addAction(Intent.ACTION_SCREEN_ON);
        boolean enable_wificontrol = getResources().getBoolean(R.bool.config_enable_screen_wifi_switch_control);
        if (enable_wificontrol) {
        registerReceiver(new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                WifiManager wm = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
                mController = new HotspotControllerImpl(context, new Handler(Looper.getMainLooper()));
                    if (Intent.ACTION_SCREEN_ON.equals(intent.getAction())) {
                        boolean mApEnable = Prefs.getBoolean(context, "SoftapEnabled", false);
                        boolean enable = Prefs.getBoolean(context, "wifiEnabled", false);
                        if (mApEnable) {
                            mController.setHotspotEnabled(true);
                            Prefs.remove(context, "SoftapEnabled");
                        }
                        if (enable) {
                        ...
                        if (wm.isWifiApEnabled()) {
                            Prefs.putBoolean(context, "SoftapEnabled", true);
                            mController.setHotspotEnabled(false);
                        }
                    }
            }
        }, bootToostFilter);
    ...
    private void startServicesIfNeeded(String[] services) {
        ...
        boolean enableKeyguardService = getResources().getBoolean(R.bool.config_enableKeyguardService);
        Log.d(TAG,"enableKeyguardService=" + enableKeyguardService);
        if (!mBootCompleted) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if(!enableKeyguardService && clsName.equals("com.android.systemui.SystemBars")  && !mBootCompleted) {
                logBootEvent("com.android.systemui.SystemBars will start later");
                continue;
            }
            if (DEBUG) Log.d(TAG, "loading: " + clsName);
    ...
    public void setHideNavigationBar(boolean b) {
