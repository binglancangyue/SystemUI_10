...
public class FlashlightControllerImpl implements FlashlightController {
    ...
    public FlashlightControllerImpl(Context context) {
        mContext = context;
        String platform = SystemProperties.get("ro.product.platform", "");
    ...
    private class InitCameraTask extends AsyncTask<Void,Void,String> {
        @Override
        ...
        @Override
        protected void onPostExecute(String cameraId) {
            if (cameraId != null) {
                mCameraId = cameraId;
                ensureHandler();
                mCameraManager.registerTorchCallback(mTorchCallback, mHandler);
