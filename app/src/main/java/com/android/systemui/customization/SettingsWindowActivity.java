...
//                        requestPermissionsTool = new RequestPermissionsTool();
    ...
    public void showPopupWindow() {
        mHandler.sendEmptyMessage(Customer.HANDLE_POP_UPDATE_DATA);
    }
    ...
    private void initPopupWindow() {
        ...
        rbCloseScreen.setOnClickListener(screenOnClickListener);
        rbBright.setOnClickListener(screenOnClickListener);
        rbScreenSaver.setOnClickListener(screenOnClickListener);
        rbTime1.setOnClickListener(screenOnClickListener);
        rbTime5.setOnClickListener(screenOnClickListener);
        rbTime30.setOnClickListener(screenOnClickListener);
        rbDay.setOnClickListener(screenOnClickListener);
        rbNight.setOnClickListener(screenOnClickListener);
        rbAuto.setOnClickListener(screenOnClickListener);
        ...
        rbRecordTime5.setOnClickListener(cameraOnClickListener);
        rbCollisionLow.setOnClickListener(cameraOnClickListener);
        rbCollisionMiddle.setOnClickListener(cameraOnClickListener);
        rbCollisionHigh.setOnClickListener(cameraOnClickListener);
        rbCollisionClose.setOnClickListener(cameraOnClickListener);
        rbADASLow.setOnClickListener(cameraOnClickListener);
        rbADASMiddle.setOnClickListener(cameraOnClickListener);
        rbADASHigh.setOnClickListener(cameraOnClickListener);

        ivFmSwitch.setOnClickListener(otherOnClickListener);
        btnDvrFormat.setOnClickListener(otherOnClickListener);
        btnDvrBT.setOnClickListener(otherOnClickListener);
        btnDvrSystemSettings.setOnClickListener(otherOnClickListener);
...
/*
    ...
    private View.OnClickListener otherOnClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            final int viewID = v.getId();
            if (viewID == R.id.iv_switch_fm) {
                mSettingsUtils.openOrCloseFM(!ivFmSwitch.isSelected());
                updateBtnFM(mSettingsUtils.getFmStatus());
            } else if (viewID == R.id.btn_dvr_bt) {
                Intent bt = new Intent(Settings.ACTION_BLUETOOTH_SETTINGS);
                startActivity(bt);
                finish();
            } else if (viewID == R.id.btn_dvr_format) {
                formatSDCard();
                finishAffinity();
            } else if (viewID == R.id.btn_dvr_settings) {
                startSettings();
            }
        }
    };
    ...
    private View.OnClickListener cameraOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            final int viewID = v.getId();
            if (viewID == R.id.rb_record_time_1) {
                setScreenControlRbCheck(rbRecordTime1, Customer.RADIO_BTN_TYPE_RECORD_TIME);
                mSharedPreferencesTool.saveRecordTime(1);
            } else if (viewID == R.id.rb_record_time_3) {
                setScreenControlRbCheck(rbRecordTime3, Customer.RADIO_BTN_TYPE_RECORD_TIME);
                mSharedPreferencesTool.saveRecordTime(3);
            } else if (viewID == R.id.rb_record_time_5) {
                setScreenControlRbCheck(rbRecordTime5, Customer.RADIO_BTN_TYPE_RECORD_TIME);
                mSharedPreferencesTool.saveRecordTime(5);
            } else if (viewID == R.id.rb_collision_value_low) {
                setScreenControlRbCheck(rbCollisionLow, Customer.RADIO_BTN_TYPE_COLLISION);
                mSharedPreferencesTool.saveCollisionLevel(1);
            } else if (viewID == R.id.rb_collision_value_middle) {
                setScreenControlRbCheck(rbCollisionMiddle, Customer.RADIO_BTN_TYPE_COLLISION);
                mSharedPreferencesTool.saveCollisionLevel(2);
            } else if (viewID == R.id.rb_collision_value_high) {
                setScreenControlRbCheck(rbCollisionHigh, Customer.RADIO_BTN_TYPE_COLLISION);
                mSharedPreferencesTool.saveCollisionLevel(3);
            } else if (viewID == R.id.rb_collision_value_close) {
                setScreenControlRbCheck(rbCollisionClose, Customer.RADIO_BTN_TYPE_COLLISION);
                mSharedPreferencesTool.saveCollisionLevel(0);
            } else if (viewID == R.id.rb_adas_value_low) {
                setScreenControlRbCheck(rbADASLow, Customer.RADIO_BTN_TYPE_ADAS);
                mSharedPreferencesTool.saveADALevel(1);
            } else if (viewID == R.id.rb_adas_value_middle) {
                setScreenControlRbCheck(rbADASMiddle, Customer.RADIO_BTN_TYPE_ADAS);
                mSharedPreferencesTool.saveADALevel(2);
            } else if (viewID == R.id.rb_adas_value_high) {
                setScreenControlRbCheck(rbADASHigh, Customer.RADIO_BTN_TYPE_ADAS);
    ...
    private View.OnClickListener screenOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            final int viewID = v.getId();
            if (viewID == R.id.rb_close_screen) {
                setScreenControlRbCheck(rbCloseScreen, Customer.RADIO_BTN_TYPE_SCREEN_CONTROL);
            } else if (viewID == R.id.rb_screen_saver) {
                setScreenControlRbCheck(rbScreenSaver, Customer.RADIO_BTN_TYPE_SCREEN_CONTROL);
                mSettingsUtils.setScreenOffTimeOut(60 * 1000);
                setScreenControlRbCheck(rbTime1, Customer.RADIO_BTN_TYPE_SCREEN_OFF_TIMEOUT);
            } else if (viewID == R.id.rb_bright) {
                setMaxScreenOffTimeOut();
            } else if (viewID == R.id.rb_time_1) {
                mSettingsUtils.setScreenOffTimeOut(60 * 1000);
                setScreenControlRbCheck(rbTime1, Customer.RADIO_BTN_TYPE_SCREEN_OFF_TIMEOUT);
            } else if (viewID == R.id.rb_time_5) {
                mSettingsUtils.setScreenOffTimeOut(300 * 1000);
                setScreenControlRbCheck(rbTime5, Customer.RADIO_BTN_TYPE_SCREEN_OFF_TIMEOUT);
            } else if (viewID == R.id.rb_time_30) {
                mSettingsUtils.setScreenOffTimeOut(1800 * 1000);
                setScreenControlRbCheck(rbTime30, Customer.RADIO_BTN_TYPE_SCREEN_OFF_TIMEOUT);
            } else if (viewID == R.id.rb_screen_control_day) {
                setDayOrNightBtnCheck(rbDay);
                sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_BRIGHTNESS, 100);
            } else if (viewID == R.id.rb_screen_control_night) {
                setDayOrNightBtnCheck(rbNight);
                sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_BRIGHTNESS, 40);
            } else if (viewID == R.id.rb_screen_control_auto) {
    ...
    public void onClick(View v) {
        final int viewID = v.getId();
        if (viewID == R.id.ll_left_wifi) {
            setLeftBtnSelected(llBtnWireless, llWireless);
        } else if (viewID == R.id.ll_left_screen_control) {
            setLeftBtnSelected(llBtnScreenControl, llScreenControl);
        } else if (viewID == R.id.ll_left_other) {
            setLeftBtnSelected(llBtnOther, llOther);
        } else if (viewID == R.id.ll_left_dvr) {
            setLeftBtnSelected(llBtnDVR, llDVR);
        } else if (viewID == R.id.ll_btn_wifi) {
            wifiOpenOrClose(!llBtnWifi.isSelected());
        } else if (viewID == R.id.ll_btn_bluetooth) {
            mSettingsUtils.openOrCloseBT(!llBtnBlueTooth.isSelected());
            sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_BTN, 4);
        } else if (viewID == R.id.ll_btn_mobile_data) {
            if (mSettingsUtils.isHasSimCard()) {
                mSettingsUtils.setDataEnabled(!mSettingsUtils.getDataEnabled());
                sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_BTN, 3);
            } else {
                ToastTool.showToast(R.string.no_sim_card);
            }
        } else if (viewID == R.id.ll_btn_gps) {
            mSettingsUtils.openGPS(!mSettingsUtils.isGpsOpen());
            sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_BTN, 2);
        } else if (viewID == R.id.ll_btn_hotspot) {
...
//                mSettingsUtils.formatMedia(StoragePaTool.getStoragePath(true));
            ...
            case R.id.rb_screen_control_night:
                setDayOrNightBtnCheck(rbNight);
                sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_BRIGHTNESS, 41);
                break;
            case R.id.rb_screen_control_auto:
                setAutoBrightnessCheck(true);
                break;
            case R.id.ll_btn_wifi:
//                sendMessageToHomeActivity(Customer.HANDLE_POP_WIFI_BTN);
                wifiOpenOrClose(!llBtnWifi.isSelected());
                break;
            case R.id.ll_btn_hotspot:
                wifiUtils.setWifiApState(!wifiUtils.isWifiApOpen());
//                sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_BTN, 1);
                break;
...
//        } else {
//            Intent intent = new Intent(Customer.ACTION_FORMAT_SD_CARD);
...
//        }
    ...
    private void setScreenControlRbCheck(RadioButton btn, int type) {
        switch (type) {
            case Customer.RADIO_BTN_TYPE_SCREEN_OFF_TIMEOUT:
                cleanFunctionRBCheck();
                rbScreenSaver.setChecked(true);
                cleanTimeRBCheck();
                break;
            case Customer.RADIO_BTN_TYPE_RECORD_TIME:
                cleanRecordTimeRBCheck();
                break;
            case Customer.RADIO_BTN_TYPE_ADAS:
                cleanADASRBCheck();
                break;
            case Customer.RADIO_BTN_TYPE_COLLISION:
                cleanCollisionRBCheck();
                break;
            case Customer.RADIO_BTN_TYPE_SCREEN_CONTROL:
                cleanFunctionRBCheck();
                break;
        }
        btn.setChecked(true);
    ...
    private void initRecordTime() {
        ...
        setScreenControlRbCheck(radioButton, Customer.RADIO_BTN_TYPE_RECORD_TIME);
    }
    ...
    private void initADASLevel() {
        ...
        setScreenControlRbCheck(radioButton, Customer.RADIO_BTN_TYPE_ADAS);
    }
    ...
    private void initCollisionLevel() {
        ...
        setScreenControlRbCheck(radioButton, Customer.RADIO_BTN_TYPE_COLLISION);
    }
...
//        seekBarBrightness.setProgress(value);
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (seekBar.getId() == R.id.sb_brightness) {
            sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_SEEK_BAR, 0);
        } else {
            sendMessageToHomeActivity(Customer.HANDLE_POP_UPDATE_SEEK_BAR, 1);
        }
        if (seekBar.getId() == R.id.sb_brightness) {
    ...
    public void openOrClose(int type, boolean state) {
        ...
        Message message = Message.obtain();
        message.what = Customer.HANDLE_POP_UPDATE_BTN_BY_LISTENER;

        switch (type) {
            case Customer.TYPE_WIFI_STATE:
                message.arg1 = 2;
                break;
            case Customer.TYPE_BT:
                message.arg1 = 4;
                break;
            case Customer.TYPE_FM:
                message.arg1 = 5;
                break;
            case Customer.TYPE_WIFI_AP:
                message.arg1 = 6;
                break;
            case Customer.TYPE_MOBILE:
                message.arg1 = 7;
                break;
            case Customer.TYPE_VOLUME:
                message.arg1 = 8;
                break;
        }
//        if (type == Customer.TYPE_WIFI_STATE) {//wifi
//            message.arg1 = 2;
//        }
//        if (type == Customer.TYPE_BT) {//bt
//            message.arg1 = 4;
//        }
//        if (type == Customer.TYPE_FM) {//fm
//            message.arg1 = 5;
//        }
//        if (type == Customer.TYPE_WIFI_AP) {//wifi ap
//            message.arg1 = 6;
//        }
//        if (type == Customer.TYPE_MOBILE) {//mobile network
//            message.arg1 = 7;
//        }
    ...
    private static class InnerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case Customer.HANDLE_POP_WIFI_BTN://wifi按钮
                    mPopupWindow.updateWifiBtn();
                    break;
                case Customer.HANDLE_POP_UPDATE_BTN:// 更新按钮状态
                    mPopupWindow.updateSettingButton(msg.arg1);
                    break;
                case Customer.HANDLE_POP_UPDATE_BRIGHTNESS:// 更新屏幕亮度seekBar Progress
                    mPopupWindow.updateBrightnessByProgress(msg.arg1);
                    break;
                case Customer.HANDLE_POP_UPDATE_SEEK_BAR:// 更新seekBar Progress
                    mPopupWindow.updateSeekBarProgress(msg.arg1);
                    break;
                case Customer.HANDLE_POP_UPDATE_DATA:
                    mPopupWindow.setData();
                    break;
                case Customer.HANDLE_POP_UPDATE_BTN_TXZ:
                    mPopupWindow.updateSettingButtonByTXZ(msg.arg1, msg.arg2);
                    break;
                case Customer.HANDLE_POP_UPDATE_BTN_BY_LISTENER:
                    switch (msg.arg1) {
                        case 2:
    ...
    private ContentObserver mGpsContentObserver = new ContentObserver(null) {
        @Override
        public void onChange(boolean selfChange) {
