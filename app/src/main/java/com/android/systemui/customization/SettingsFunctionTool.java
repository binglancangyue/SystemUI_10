...
//        String s = NumberFormat.getPercentInstance().format(getCurrentBrightness());
        String s = NumberFormat.getPercentInstance().format(getScreenBrightnessPercentageValueForAndroid10());
        Log.d(TAG, "getScreenBrightnessPercentageValue: " + s);
        s = s.replace("%", "");
...
//            modifyScreenBrightness(brightnessValue);
            progressChangeToBrightnessForAndroid10(progress);
        } catch (Exception e) {
            Log.e(TAG, "progressChangeToBrightness Exception: " + e.getMessage());
        }
    }

    public void progressChangeToBrightnessForAndroid10(int progress) {
        int value = GAMMA_SPACE_MAX * progress / 100;
        Log.d(TAG, "progressChangeToBrightnessForAndroid10: value" + value + " progress");
        int screenValue = convertGammaToLinear(value, mMinBrightness, mMaxBrightness);
        Log.d(TAG, "progressChangeToBrightnessForAndroid10: brightnessValue " + screenValue);
        modifyScreenBrightness(screenValue);
    }
...
//                id = volumeInfo.getDiskId();
                ...
                Log.d(TAG, "VolumeInfo getType " + volumeInfo.getType() + " getDiskId " + volumeInfo.getDiskId());
            }
        }
        return diskinfo;
    }
    ...
    public double getScreenBrightnessPercentageValueForAndroid10() {
        int systemValue = Settings.System.getInt(contentResolver,
                Settings.System.SCREEN_BRIGHTNESS, mMinBrightness);
        int value = convertLinearToGamma(systemValue, mMinBrightness, mMaxBrightness);
        Log.d(TAG, "getCurrentBrightness: value " + value + " systemValue " + systemValue);
